...
export default function Admin() {
  ...
  const addProduct = async () => {
    if (!newProduct.name || !newProduct.price) return alert("Fill in name and price.");
  ...
  const saveEdit = async () => {
    await updateDoc(doc(db, "products", editing.id), { name: editing.name, price: editing.price, description: editing.description });
    setEditing(null);
    fetchData();
  };
          ...
          <div>
            <div style={{ background: "#fff", padding: 24, borderRadius: 12, border: "1px solid #e5e5e5", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <input placeholder="Product name" value={newProduct.name} onChange={e => setNewProduct({ ...newProduct, name: e.target.value })} style={{ flex: 1, minWidth: 150, padding: "10px 14px", borderRadius: 8, border: "1px solid #e5e5e5", fontSize: 14 }} />
                <input placeholder="Price (e.g. 800)" value={newProduct.price} onChange={e => setNewProduct({ ...newProduct, price: e.target.value })} style={{ flex: 1, minWidth: 120, padding: "10px 14px", borderRadius: 8, border: "1px solid #e5e5e5", fontSize: 14 }} />
                <input placeholder="Description" value={newProduct.description} onChange={e => setNewProduct({ ...newProduct, description: e.target.value })} style={{ flex: 2, minWidth: 200, padding: "10px 14px", borderRadius: 8, border: "1px solid #e5e5e5", fontSize: 14 }} />
                <button onClick={addProduct} style={{ background: "#3B6D11", color: "#fff", border: "none", padding: "10px 24px", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Add</button>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
              {products.map(p => (
                <div key={p.id} style={{ background: "#fff", padding: 20, borderRadius: 12, border: "1px solid #e5e5e5" }}>
                  {editing?.id === p.id ? (
                    <div>
                      <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #e5e5e5", marginBottom: 8, boxSizing: "border-box" }} />
                      <input value={editing.price} onChange={e => setEditing({ ...editing, price: e.target.value })} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #e5e5e5", marginBottom: 8, boxSizing: "border-box" }} />
                      <input value={editing.description} onChange={e => setEditing({ ...editing, description: e.target.value })} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #e5e5e5", marginBottom: 12, boxSizing: "border-box" }} />
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={saveEdit} style={{ flex: 1, background: "#3B6D11", color: "#fff", border: "none", padding: "8px", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Save</button>
                        <button onClick={() => setEditing(null)} style={{ flex: 1, background: "#e5e5e5", color: "#333", border: "none", padding: "8px", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div>
                      ...
                      <div style={{ fontWeight: 700, color: "#3B6D11", marginBottom: 16 }}>₹{p.price}</div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setEditing(p)} style={{ flex: 1, background: "#EAF3DE", color: "#3B6D11", border: "none", padding: "8px", borderRadius: 8, fontWeight: 600, cursor: "pointer" }}>Edit</button>
                        <button onClick={() => deleteProduct(p.id)} style={{ flex: 1, background: "#FCEBEB", color: "#A32D2D", border: "none", padding: "8px", borderRadius: 8, fontWeight: 600, cursor: "pointer" }}>Delete</button>
                      </div>
                    </div>
                  )}
                </div>
